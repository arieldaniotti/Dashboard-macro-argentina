...
def run_pipeline():
    hoy     = datetime.today()
    hace_2a = hoy - timedelta(days=730)
    print(f'Iniciando pipeline — {hoy.strftime('%d/%m/%Y %H:%M')}')
    ...
    cpi  = pdr.get_data_fred('CPIAUCSL', start=hace_2a).dropna()
    fed  = pdr.get_data_fred('FEDFUNDS', start=hace_2a).dropna()
    gs10 = pdr.get_data_fred('GS10',     start=hace_2a).dropna()
    gs2  = pdr.get_data_fred('GS2',      start=hace_2a).dropna()
    ...
    print(f'✓ Guardado — fila {len(hoja.get_all_values())}')
